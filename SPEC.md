...
同じスタック（Python scraper + GitHub Actions + GitHub Pages + React Native）を流用できる候補。

### 裁判系の横展開

| アイデア | 概要 |
|----------|------|
| 全国版 | 対象を近畿8庁から全国50庁以上に拡張 |
| 判決結果の記録 | 裁判員裁判の判決情報（量刑等）を蓄積して統計表示 |

### 公共情報アプリ

| アイデア | 概要 | 備考 |
|----------|------|------|
| 官報アプリ | 毎日発行される官報（会社設立・破産・法令）を検索しやすく表示 | |
| 議会傍聴スケジュール | 都道府県・市区町村議会の本会議・委員会の開催日程 | 自治体ページの構造が多様でスクレイピングが困難。過去に試みて断念した経緯あり |
| パブリックコメント締切通知 | 国や自治体の意見募集の締切をプッシュ通知 | |

### 地域密着型

| アイデア | 概要 |
|----------|------|
| 滋賀県の入札情報 | 公共工事の入札公告をまとめて閲覧 |
| 地域の空き家バンク | 自治体バラバラの空き家情報を一画面に集約 |
